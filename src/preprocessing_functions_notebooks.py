    ...
    for tile_id in tqdm(tile_ids):  # iterate through each tile id
        ...
        for field_id in np.unique(
            ...
            if len(field_label) == 1:  # ignore fields with multiple labels
                ...
                dates.append(
                    tile_dates
                )  # add the dates which are available for the current tile
    df = pd.DataFrame(
        dict(field_id=fields, tile_id=tiles, label=labels, dates=dates)
...
def get_clm(bands):
    ...
    cloud = np.expand_dims(
        bands_T[len(bands_T) - 1], axis=0
...
def calculate_band_mode(band):
    ...
    mode = np.squeeze(mode[0], axis=0).transpose(
        1, 0
