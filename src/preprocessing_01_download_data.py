...
class PreprocessingDownload:
    ...
    def __init__(self, ROOT_DIR: str) -> None:
        ...
        self.DOWNLOAD_S1 = (
            False  # If you set this to true then the Sentinel-1 data will be downloaded
        )
    ...
    def download_archive(self, archive_name: str):
        ...
        download_url = (
            f"https://radiant-mlhub.s3.us-west-2.amazonaws.com/archives/{archive_name}"
        )
        ...
        for item_link in tqdm(item_links):
            ...
            for link in item["links"]:
                ...
                if source_item_id.find("_s1_") > 0 and not self.DOWNLOAD_S1:
                    ...
                    for band in ["VV", "VH"]:
                        asset_path = Path(
                            f"{self.FOLDER_BASE}_{split}_source_s1/{source_item_id}/{band}.tif"
                        ).resolve()
                        date = "-".join(source_item_id.split("_")[10:13])

                        rows.append(
                            [tile_id, f"{date}T00:00:00Z", "s1", band, asset_path]
                        )
                ...
                if source_item_id.find("_s2_") > 0:
                    for band, download in self.DOWNLOAD_S2.items():
                        ...
                        asset_path = Path(
                            f"{self.FOLDER_BASE}_{split}_source_s2_{band}/{source_item_id}_{band}.tif"
                        ).resolve()
                        date = "-".join(source_item_id.split("_")[10:13])
                        rows.append(
                            [tile_id, f"{date}T00:00:00Z", "s2", band, asset_path]
                        )

        return pd.DataFrame(
            rows,
            columns=["tile_id", "datetime", "satellite_platform", "asset", "file_path"],
        )
    ...
    def start_download(self):
        ...
        print(
            f"Saving the image info into a CSV file to {self.IMAGE_DIR}/images_info_data.csv"
        )
        df_images.to_csv("images_info_data.csv", index=False)

        # save the used bands into a pickle file
        print(
            f"Saving the used band info into a pkl file to {self.IMAGE_DIR}/used_bands.pkl"
        )
