...
def get_cloudmask_frame(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    unclouded = df[df["CLM"] == 0].groupby(column)[column].count()
    noinfo = df[df["CLM"] == 255].groupby(column)[column].count()
...
def get_count_per_field_id(df: pd.DataFrame, column: str) -> pd.DataFrame:
...
def plot_confusion_matrix(xgb_cm: confusion_matrix, labels: list):
    ...
    Args:
        ...
        labels (list): List of the class labels.
    """
...
def get_label_accuracies(xgb_cm: confusion_matrix, labels: list) -> pd.DataFrame:
    """Gives back a data frame with the accuracy for each class label
        and the class it is most often confused with, with a percentage of how often.

    Args:
        xgb_cm (confusion_matrix): The confusion matrix from which the information is derived.
        labels (list): List of the class labels.

    Returns:
        pd.DataFrame: Data frame containing the accuracy and confusion information.
    """
...
def plot_label_accuracy(df: pd.DataFrame):
    """Plot the accuracy per label as a bar chart.

    Args:
        df (pd.DataFrame): Data frame generated by the get_label_accuracies function.
    """
...
def plot_feature_importance(
    importance: list, feature_names: list, model_name: str, num_features=None
):
    """_summary_

    Args:
        importance (list): A list of the feature importance values, given by the model.
        feature_names (list): A list of the feature names (X.columns could be used).
        model_name (str): The name of the used model
        num_features (int, optional): The number of the most important features to be plotted. Defaults to None.
    """
