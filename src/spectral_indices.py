...
# MNSI
def cal_MNSI(
    Band3: pd.Series, Band4: pd.Series, Band6: pd.Series, Band9: pd.Series
) -> pd.Series:
...
# GARI
def cal_GARI(
    Band1: pd.Series, Band3: pd.Series, Band5: pd.Series, Band9: pd.Series
) -> pd.Series:
...
def drop_na(df: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
    ...
    if verbose:
        ...
        print(
            f"Precentage of remaining Data:  {round((df_wo_NA.shape[0] / df.shape[0]) * 100, 3)} %"
        )
