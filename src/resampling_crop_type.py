...
class ResamplingProcess:
    ...
    def get_train_test_data(self) -> Union[pd.DataFrame, pd.DataFrame]:
        # load the train and test data.
        df = pd.read_csv(f"{self.DATA_DIR}/data_after_FE.csv")
        df_train, df_test = train_test_split_fields(df, train_size=0.7)
        return df_train, df_test

    def merge_crop_labels(self, df_train, df_test) -> Union[pd.DataFrame, pd.DataFrame]:
    ...
    def get_label_ranking(self, df_label_comb: pd.DataFrame) -> pd.DataFrame:
        # rank the labels by their count
        label_ranking = (
            df_label_comb[["label", "field_id"]]
            .groupby("label")
            .count()
    ...
    def get_base_label_index(self, label_ranking: pd.DataFrame) -> int:
    ...
    def get_base_label_df(
        self, df_label_comb: pd.DataFrame, label_ranking: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def get_resampling_size(self, label_ranking: pd.DataFrame) -> int:
    ...
    def get_labels_to_resample(
        self, label_ranking: pd.DataFrame
    ...
    def do_resampling(
        self,
        df_label_comb: pd.DataFrame,
        oversampling: bool,
        labels_to_resample: pd.Series,
        resampling_size: int,
    ) -> pd.DataFrame:
        # create a data subset for the labels, which we want to resample
        df_resampling = df_label_comb.loc[
            df_label_comb["label"].isin(labels_to_resample)
        ]
    ...
    def save_train_test_data(
        ...
        df_base_label: pd.DataFrame,
        df_test_label_comb: pd.DataFrame,
    ):
        # create a balanced dataset by merging the 3 subsset we created above
        df_resampled = pd.concat(
            [df_oversampled, df_undersampled, df_base_label], axis=0
        )
        ...
        rows_to_remove_test = df_test_label_comb.index[
            np.isinf(df_test_label_comb).any(1)
        ]
        df_test_label_comb.drop(index=rows_to_remove_test, inplace=True)
        # save the resample data set as csv file
        df_resampled.to_csv(
            f"{self.DATA_DIR}/Train_Dataset.csv", index=False
        )  # create a csv file
        df_test_label_comb.to_csv(f"{self.DATA_DIR}/Test_Dataset.csv", index=False)
        print(
            f"Save the train and test data to {self.DATA_DIR} as Train_Dataset.csv and Test_Dataset.csv"
        )
    ...
    def start_resampling(self):
        ...
        df_train_label_comb, df_test_label_comb = self.merge_crop_labels(
            df_train, df_test
        )
        ...
        df_base_label = self.get_base_label_df(df_train_label_comb, 
            label_ranking
        )
        # get the labels to over- and undersample
        labels_to_oversample, labels_to_undersample = self.get_labels_to_resample(
            label_ranking
        )
        ...
        self.save_train_test_data(
            df_oversampled, df_undersampled, df_base_label, df_test_label_comb
        )
