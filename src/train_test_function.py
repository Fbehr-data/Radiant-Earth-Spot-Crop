...
def train_test_split_fields(
    df: pd.DataFrame, train_size=0.7, random_state=42
) -> pd.DataFrame:
    ...
    train_fields = np.random.choice(
        df["field_id"].unique(), int(n_fields * train_size), replace=False
    )
    test_fields = df["field_id"].unique()[
        ~np.in1d(df["field_id"].unique(), train_fields)
    ]
